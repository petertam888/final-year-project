# -*- coding: utf-8 -*-
"""
Created on Thu Mar 25 06:09:03 2021
...
for distance in distance_case:
            ...
            filename=os.path.join(os.path.sep, 'E:\\','fyp','input_data','4_50km6SNR_18SNR','actual value vs true value '+str(distance)+'km_18.csv')
            ...
            def show_train_history(train_history, train, validation,inter):
                ...
                fig=plt.savefig(os.path.join('E:/fyp/report/present/traininghistory/final',str(distance) + ' 18SNR '+'  ' +str(inter)+'.svg'), format='svg', dpi=1200)
                plt.close(fig)
            ...
            for train, test in kfold.split(x_input, y1):
                ...
                train_history=model.fit(x_input[train],y1[train], epochs=n_epoch,validation_split=0.2, batch_size=m,verbose=2,callbacks=keras.callbacks.ModelCheckpoint(os.path.join('E:/fyp/report/present/trained_model/final'+str(distance)+'km_model_+ 18dB(SNR).h5'), monitor='val_accuracy', verbose=1, save_best_only=True, mode='auto'))
            ...
            fig=plt.savefig(os.path.join('E:/fyp/report/present/kcross/final',nameline+'.svg'), format='svg', dpi=1200)
            plt.close(fig)
